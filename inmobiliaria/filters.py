...
class PropiedadFilter(df.FilterSet):
    ...
    banos_min = df.NumberFilter(field_name="baños", lookup_expr="gte")  
    ...
    orientacion = df.CharFilter(field_name="orientación", lookup_expr="iexact")

    class Meta:
        model = Propiedad
        fields = ["tipo", "estado", "ciudad", "orientación"]